...
.PHONY : keyscape abc lilypond mei midi midi-norep musedata musicxml notearray pdf-lilypond pdf-musedata reference-edition
...
ifeq ($(shell which curl),)
	...
	for file in kern/*.krn; do \
	   ...
	   curl \
	      "http://kern.humdrum.org/data?l=folk/sioux&file=$$TBASE.krn&format=pdf" \
...
##############################
#
# make abc -- Create abc files.
# Output is stored in a directory called "abc".
#

abc: humdrum-extras-check
	mkdir -p abc
	hum2abc kern/*.krn > abc/densmore-teton-sioux.abc
# To create individual files for each song:
#	for file in kern/*.krn;						\
#	do								\
#	   TBASE=`basename $$file .krn`;				\
...
ifeq ($(wildcard .git),)
	...
	@echo "***    [0;32mgit clone https://github.com/craigsapp/densmore-teton-sioux[0;31m"
	@echo "[0m"
	exit 1
endif
